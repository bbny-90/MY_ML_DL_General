...
def test_second_grad_scalar():
    ...
    #
    dydx_autodiff = get_grad_scalar(y, x)
    ddydxx_autodiff = get_grad_scalar(dydx_autodiff, x).detach().numpy()
...
def test_grad_vector_2dim():
    ...
    grad_y = get_gradient_vector(y, x).detach().numpy()
...
def test_divergence_tensor_2dim():
    ...
    y1 = x[:, 0]**3 + x[:, 1]
    y = torch.stack([y0, y1], dim=1)
    grad_y = get_gradient_vector(y, x)
    div_grad_y = get_divergence_tensor(grad_y, x).detach().numpy()
    x = x.detach().numpy()
...
def test_biharmonic_2dim():
    ...
    grad_u = get_gradient_vector(u, x)
    eps = (grad_u + grad_u.transpose(1, 2)) * 0.5
    div_eps = get_divergence_tensor(eps, x).detach().numpy()
